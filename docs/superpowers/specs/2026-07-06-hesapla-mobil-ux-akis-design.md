...
**Bilinen test kapsamı borcu (kabul edilmiş, engelleyici değil):** Reveal-gate durum makinesinin (`isResultsRevealed`) kendisini uçtan uca (gerçek render + tıklama + `data-revealed` DOM özniteliğinin geçişini assert etme) egzersiz eden otomatik bir entegrasyon testi yok. Mevcut kapsam `pageStyles.scope.test.ts`'deki CSS-pozisyonel guard testlerinden (CSS kablolamasının doğru olduğunu doğrular, ama `page.tsx`'i hiç render etmez) ve her görevin `.superpowers/sdd/task-N-report.md`'sinde kayıtlı manuel Playwright doğrulamalarından geliyor — CI tarafından zorunlu kılınan bir testten değil. Bunun nedeni: `page.tsx` için RTL testi yazmak `next-auth`'un `useSession`'ını, dinamik `@react-pdf/renderer` import'unu ve Chart.js tabanlı grafik bileşenlerini mock'lamayı gerektiriyor — bu dosya için bu codebase'de daha önce yapılmamış, hafif olmayan yeni bir test altyapısı yatırımı (`hesapla` altında henüz bir `page.test.tsx` yok). Bu, gelecekte `page.tsx` test altyapısı (next-auth/PDF/Chart.js mock'ları) kurulursa ele alınabilecek bir aday takip işi olarak bilinçli şekilde ertelenmiştir; bu branch'in birleştirilmesini engellemez.
